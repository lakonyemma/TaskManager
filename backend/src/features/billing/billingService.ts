...
export const getSubscription = async (userId: string) => {
  const result = await prisma.$queryRawUnsafe<any[]>(
    `SELECT "id", "userId", "plan", "status", "provider", "paymentMethod", "currency", "amountMinor", "trialStartAt", "trialEndAt", "currentPeriodStart", "currentPeriodEnd", "cancelAtPeriodEnd", "canceledAt", "gracePeriodEnd", "providerCustomerId", "providerSubscriptionId", "providerTxRef", "createdAt", "updatedAt"
     FROM "BillingSubscription" WHERE "userId" = $1 LIMIT 1`,
    userId,
  );
...
export const ensureSubscription = async (userId: string) => {
  ...
  const result = await prisma.$queryRawUnsafe<any[]>(
    `INSERT INTO "BillingSubscription" ("userId", "plan", "status") VALUES ($1, 'FREE', 'FREE')
...
export const startTrial = async (userId: string) => {
  ...
  const result = await prisma.$queryRawUnsafe<any[]>(
    `UPDATE "BillingSubscription"
     SET "plan"='PREMIUM', "status"='TRIALING', "trialStartAt"=$2, "trialEndAt"=$3,
         "currentPeriodStart"=$2, "currentPeriodEnd"=$3, "amountMinor"=0, "updatedAt"=CURRENT_TIMESTAMP
...
export const activatePaidSubscription = async (params: {
  ...
  await prisma.$executeRawUnsafe(
    `UPDATE "BillingSubscription"
     SET "plan"='PREMIUM', "status"='ACTIVE', "provider"=$2, "paymentMethod"=$3,
         "currency"=$4, "amountMinor"=$5, "currentPeriodStart"=$6, "currentPeriodEnd"=$7,
         "gracePeriodEnd"=NULL, "cancelAtPeriodEnd"=false, "canceledAt"=NULL,
         "providerTxRef"=$8, "providerSubscriptionId"=$9, "providerCustomerId"=$10,
         "updatedAt"=CURRENT_TIMESTAMP WHERE "id"=$1`,
    subscriptionId, params.provider, params.paymentMethod, params.currency, params.amountMinor,
    now, end, params.txRef, params.providerSubscriptionId ?? null, params.providerCustomerId ?? null,
  );

  await prisma.$executeRawUnsafe(
    `INSERT INTO "BillingPayment" ("subscriptionId", "userId", "provider", "providerPaymentId", "txRef", "amountMinor", "currency", "paymentMethod", "status", "rawData")
     VALUES ($1,$2,$3,$4,$5,$6,$7,$8,'SUCCEEDED',$9::jsonb)
     ON CONFLICT ("providerPaymentId") DO UPDATE SET "status"='SUCCEEDED', "rawData"=EXCLUDED."rawData"`,
    subscriptionId, params.userId, params.provider, params.providerPaymentId, params.txRef,
    params.amountMinor, params.currency, params.paymentMethod, JSON.stringify(params.rawData ?? {}),
  );
...
export const markPastDue = async (userId: string) => {
  ...
  await prisma.$executeRawUnsafe(
    `UPDATE "BillingSubscription" SET "status"='PAST_DUE', "gracePeriodEnd"=$2, "updatedAt"=CURRENT_TIMESTAMP WHERE "userId"=$1 AND "plan"='PREMIUM'`,
    userId, end,
  );
};
...
export const cancelSubscription = async (userId: string) => {
  await prisma.$executeRawUnsafe(
    `UPDATE "BillingSubscription" SET "cancelAtPeriodEnd"=true, "canceledAt"=CURRENT_TIMESTAMP, "updatedAt"=CURRENT_TIMESTAMP WHERE "userId"=$1`,
    userId,
  );
...
export const enforceExpiredSubscriptions = async () => {
  await prisma.$executeRawUnsafe(
    `UPDATE "BillingSubscription"
     SET "plan"='FREE', "status"='EXPIRED', "provider"=NULL, "paymentMethod"=NULL,
         "amountMinor"=0, "updatedAt"=CURRENT_TIMESTAMP
     WHERE "plan"='PREMIUM' AND (
       ("status"='TRIALING' AND "trialEndAt" < CURRENT_TIMESTAMP) OR
       ("status"='ACTIVE' AND "cancelAtPeriodEnd"=true AND "currentPeriodEnd" < CURRENT_TIMESTAMP) OR
       ("status"='PAST_DUE' AND "gracePeriodEnd" < CURRENT_TIMESTAMP)
     )`,
  );
};
...
export const getEntitlements = async (userId: string) => {
  ...
  const workspaceResult = await prisma.$queryRawUnsafe<any[]>(
    `SELECT COUNT(*)::int AS count FROM "WorkspaceMember" WHERE "userId"=$1 AND "role"='OWNER'`,
    userId,
  );
